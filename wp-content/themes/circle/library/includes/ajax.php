        ...
        while ($posts->have_posts()):
            ...
            if ($_element['month-year'] !== $last_array_element['month-year']) {
                if ($current_date != $previous_date):
                    ...
                    $total_post_no = kopa_total_post_count_by_month(get_the_date('m'), get_the_date('Y'), $query_args['categories']);
                    ?>
...
if (!function_exists('load_more_articles_custom_df')) {
    ...
    function load_more_articles_custom_df() {
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST['kopa_post_id_string'])) {
                $posts_ids = $_POST['kopa_post_id_string'];
            }
            $posts_ids_arr = explode(',', $posts_ids);
            if (isset($_POST['stored_month_year'])) {
                $stored_month_year = $_POST['stored_month_year'];
            }
            echo kopa_get_articles_custom_df($cat_name, $posts_ids_arr, $stored_month_year);
        }
...
function kopa_get_articles_custom_df($term_name, $posts_ids_arr, $stored_month_year) {
...
if($df_post_count != 0){
        $month_year = json_decode(stripslashes($stored_month_year), true);
        $post_id_array = array();
        foreach ($posts_ids_arr as $kopa_post_key => $kopa_post_id) {
            array_push($post_id_array, $kopa_post_id);
        }
        foreach ($df_blogs as $df_blog) {
            ...
            $query_args['orderby'] = 'latest';
            $query_args['post__not_in'] = $posts_ids_arr;
            $posts = kopa_widget_article_build_query($query_args);
            while ($posts->have_posts()):
                $posts->the_post();
                $post_id = get_the_ID();
