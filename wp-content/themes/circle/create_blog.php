...
function show_blog_form($blogname = '', $blog_title = '', $errors = '') {
	...
	<?php }
        echo '<p>No spaces and only lowercase letters (a-z) and numbers are allowed.</p><br/>';
	if ( !is_subdomain_install() )
...
function signup_another_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	<form id="setupform" method="post" action="">
		...
		<p class="submit"><input type="submit" name="submit" class="submit pop_btn" value="<?php esc_attr_e( 'Create Blog' ) ?>" /></p>
	</form>
	<?php
}
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array() ) {
	...
	<p>
		<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site.' ), $domain.$path, $domain.$path) ?>
	</p>
	<?php
	/**
...
function signup_user($user_name = '', $user_email = '', $errors = '') {
	...
	<form id="setupform" method="post" action="<?php echo $_SERVER['PHP_SELF']?>">
...
if ( is_super_admin() )
	echo '<div class="mu_alert">' . sprintf( __( 'Greetings Site Administrator! You are currently allowing &#8220;%s&#8221; registrations.' ), $i18n_signup[$active_signup]) . '</div>';
