...
function kopa_comment_form_args() {
    ...
    $args = array(
        ...
        'must_log_in' => '<p class="alert">' . sprintf(__('You must be %1$s to post a comment. Not yet a member? Register <a href="%2$s">here.</a>', kopa_get_domain()),(!is_user_logged_in())? do_shortcode('[wp-modal-login login_text="logged in"]'):'', get_permalink( 707 )) . '</p><!-- .alert -->',
        'logged_in_as' => '<p class="log-in-out">' . sprintf(__('Logged in as <a href="http://dailyfoodies.com/my-account/my-profile/" title="%2$s">%2$s</a>.', kopa_get_domain()), admin_url('profile.php'), esc_attr($user_identity)) . ' <a href="' . wp_logout_url(get_permalink()) . '" title="' . esc_attr__('Log out of this account', kopa_get_domain()) . '">' . __('Log out &raquo;', kopa_get_domain()) . '</a></p><!-- .log-in-out -->',
