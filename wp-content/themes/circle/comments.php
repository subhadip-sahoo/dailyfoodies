...
function kopa_comment_form_args() {
    ...
    $comment_field = '<div class="clear"></div><div class="comment-left">
                        <p class="input-block">               
                        <label class="required" for="comment_name" >' . __("Name <span>(required):</span>", kopa_get_domain()) . '</label>
                        <input type="text" name="author" id="comment_name" readonly value="' . esc_attr($user_identity) . '">                                               
                        </p>
    ...
    $args = array(
        ...
        'must_log_in' => '<p class="alert">' . sprintf(__('You must be %1$s to post a comment. Not yet a member? Register <a href="%2$s">here.</a>', kopa_get_domain()),(!is_user_logged_in())? do_shortcode('[wp-modal-login login_text="logged in"]'):'', get_permalink( 709 )) . '</p><!-- .alert -->',
        'logged_in_as' => '<p class="log-in-out">' . sprintf(__('Logged in as <a href="%1$s" title="%2$s">%2$s</a>.', kopa_get_domain()), 'http://dailyfoodies.com/my-account/edit-account-info/', esc_attr($user_identity)) . ' <a href="http://dailyfoodies.com/my-account/logout/" title="' . esc_attr__('Log out of this account', kopa_get_domain()) . '">' . __('Log out &raquo;', kopa_get_domain()) . '</a></p><!-- .log-in-out -->',
