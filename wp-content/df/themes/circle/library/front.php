...
function kopa_breadcrumb() {
    if (is_main_query()) {
        ...
        if (is_home()) {
            ...
            $breadcrumb.= $prefix . sprintf('<span class="%1$s">%2$s</span>', $current_class, ucfirst(get_option('blogname')));
        } else if (is_tag()) {
            $breadcrumb.= $breadcrumb_home;
...
function kopa_total_post_count_by_month_cat($month, $year, $site_categories) {
    $args = array(
        'monthnum' => (int) $month,
        'year' => (int) $year,
        'category__in' => $site_categories
    );
